...
export default (
  _api: <T = any>(options: AxiosRequestConfig) => Promise<ISrkResponse<T>>,
  username: string,
  id: string
) => {
  return new Promise<ISrkResponse<IDestinationListPayload>>((resolve) =>
    resolve({
      ok: true,
      payload: {
        id,
  ...
  // return api<IDestinationListPayload>({
  //   url: "/api/lists/" + username + "/" + id,
  // });
};
